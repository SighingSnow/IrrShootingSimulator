...
void eventManager::CreateGUI()
{
    ...
    guix.logo = env->addImage(driver->getTexture("logo.png"),position2d<s32>(0,0),true,0);
    guix.Window = env->addWindow ( rect<s32> ( 0, 0, dim.Width, dim.Height ), false, L"IrrShootingSim",0,-1);
    guix.Window->setToolTipText ( L"Just have fun here" );
    
    guix.cover = env->addImage(driver->getTexture("cover.jpg"),position2d<s32>(0,0),true,0);
    

    guix.singlePlayer = env->addButton(rect<s32>( dim.Width - 250, dim.Height - 350, dim.Width - 50, dim.Height - 300 ), guix.Window,-1, L"Single Player");
    guix.singlePlayer->setToolTipText(L"Play immediately with AI");
    
    guix.multiPlayer = env->addButton(rect<s32>( dim.Width - 250, dim.Height - 250, dim.Width - 50, dim.Height - 200 ), guix.Window,-1, L"Multi Player");
    guix.multiPlayer->setToolTipText(L"Play with human when connected");
    
    // function that cannot worked here
...
void eventManager::CreateRoom()
{
    ...
    room.Play = env->addButton(rect<s32>( dim.Width - 250, dim.Height - 550, dim.Width - 50, dim.Height - 450 ), room.Window,1, L"Play");
    room.Play->setToolTipText(L"Double Clicked Please");
    
    room.enterRoom = env->addButton(rect<s32>( dim.Width - 500, dim.Height - 350, dim.Width - 350, dim.Height - 300 ), room.Window,1, L"Enter Room");
    room.enterRoom->setToolTipText( L"Please enter ip of the server in the below textbox" );
    
    room.createRoom = env->addButton(rect<s32>( dim.Width - 700, dim.Height - 350, dim.Width - 550, dim.Height - 300 ), room.Window,1, L"Create Room");
    room.createRoom->setToolTipText( L"You can enter the game directly and wait for others");
...
void eventManager::SetGUIActive(s32 command)
{
    ...
    switch ( command )
    {
        case 0: Game->guiActive = 0; break;
        case 1: Game->guiActive = 1; break;
        case 2: Game->guiActive ^= 1; break;
    }
...
void eventManager::SetRoomActive(s32 command)
{
    ...
    switch ( command )
    {
        case 0: Game->roomActive = 0;  break;
        case 1: Game->roomActive = 1;  break;
        case 2: Game->roomActive ^= 1; break;
    }
...
bool eventManager::OnEvent(const irr::SEvent& event) {
    if(event.EventType == EET_GUI_EVENT){
        if(Game->guiActive){
            ...
            if(event.GUIEvent.Caller == room.enterRoom && event.GUIEvent.EventType == gui::EGET_BUTTON_CLICKED){
                SetRoomActive(0);
                device->getCursorControl()->setVisible(false);
                
                netMgr = new Client();
                netMgr->initNet();
                netMgr->isServer = false;
                Game->netEnable = 1;
                
            }
            if(event.GUIEvent.Caller == room.createRoom && event.GUIEvent.EventType == gui::EGET_BUTTON_CLICKED){
                    // create server and enter room
                SetRoomActive(0);
                device->getCursorControl()->setVisible(false);
                
                netMgr = new Server();
                netMgr->initNet();
                netMgr->isServer = true;
                Game->netEnable = 1;
            }
        }
    }
    ...
    if (event.EventType == irr::EET_KEY_INPUT_EVENT && !Game->guiActive && !Game->roomActive) {
    ...
    if(event.EventType == irr::EET_MOUSE_INPUT_EVENT && !Game->guiActive && !Game->roomActive) {
